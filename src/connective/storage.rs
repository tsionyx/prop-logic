...
/// This type stores all `TruthFn`-s for a given `ARITY`.
///
/// Also, it allows to compare the given generic function
/// (identified by their truth tables, TO BE DONE) with the [concrete ones][TruthFn].
pub type AllFunctions<const ARITY: usize> = CheckedStorage<ARITY, D, &'static dyn TruthFn<ARITY>>;
...
pub const NULLARY_FUNCTIONS: AllFunctions<0> = CheckedStorage::new([
    &functions::Falsity as &'static dyn TruthFn<0>,
    &functions::Truth,
]);
...
pub const UNARY_FUNCTIONS: AllFunctions<1> = CheckedStorage::new([
    &functions::Falsity as &'static dyn TruthFn<1>,
...
pub const BINARY_FUNCTIONS: AllFunctions<2> = CheckedStorage::new([
    &functions::Falsity as &'static dyn TruthFn<2>, // 0 0 0 0
