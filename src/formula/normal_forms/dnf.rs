...
impl<T> Evaluable for NormalForm<T> {
    ...
    fn terminal(value: bool) -> Self {
        if value {
            Self::new(Some(ProductTerm::tautology()))
        } else {
            Self::contradiction()
        }
    }
    ...
    T: PartialEq,
{
        ...
        match formula {
            ...
            Formula::And(f1, f2) => {
                ...
                let result = term1.and_then(|term1| {
                    term2.and_then(|term2| Self::and(term1.into_iter().chain(term2).collect()))
                });
                Ok(result)
            }
            Formula::Or(_, _)
    ...
    /// Create a `ProductTerm` from a number of literals.
    ///
    /// # Return
    /// - `None` corresponds to constant _Falsity_;
    /// - `Some` but empty `ProductTerm` corresponds to _Truth_.
    pub fn and(conjuncts: Vec<Literal<T>>) -> Option<Self> {
        let mut unique: Vec<Literal<T>> = Vec::with_capacity(conjuncts.len());
    ...
    T: PartialEq,
{
    fn convert_prepared(formula: Formula<T>) -> Result<Self, Error> {
        let nf = if let Formula::Or(f1, f2) = formula {
            let nf1 = Self::convert_prepared(*f1)?;
            let nf2 = Self::convert_prepared(*f2)?;

            Self::or(nf1.into_iter().chain(nf2).collect())
        } else {
            let term = ProductTerm::convert_prepared(formula)?;
            Self::new(term)
        };
        Ok(nf)
    }
    ...
    pub fn or(disjuncts: Vec<ProductTerm<T>>) -> Self {
        let mut unique: Vec<ProductTerm<T>> = Vec::with_capacity(disjuncts.len());
...
mod tests {
    ...
    fn conjunction_with_truth() {
        ...
        assert_eq!(
            nf,
            NormalForm::new(Some(ProductTerm::new(Some('a'.into()))))
        );
    }
