...
pub enum NormalForm<T> {
    ...
    /// A number of conjunctions of the sub-formulas.
    ///
    /// The empty set represents a [`truth`][Evaluable::tautology] value.
    And(Vec<Self>),

    /// A number of disunctions of the sub-formulas.
    ///
    /// The empty set represents a [`false`][Evaluable::contradiction] value.
    Or(Vec<Self>),
}
