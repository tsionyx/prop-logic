...
impl<T> Formula<T> {
        ...
        F: FnMut(Self) -> Self,
    {
        match self {
            ...
            Self::Not(f) => Self::Not(Box::new(transform(*f))),
            Self::And(f1, f2) => Self::And(Box::new(transform(*f1)), Box::new(transform(*f2))),
            Self::Or(f1, f2) => Self::Or(Box::new(transform(*f1)), Box::new(transform(*f2))),
            Self::Xor(f1, f2) => Self::Xor(Box::new(transform(*f1)), Box::new(transform(*f2))),
            Self::Implies(f1, f2) => {
                Self::Implies(Box::new(transform(*f1)), Box::new(transform(*f2)))
            }
            Self::Equivalent(f1, f2) => {
                Self::Equivalent(Box::new(transform(*f1)), Box::new(transform(*f2)))
            }
