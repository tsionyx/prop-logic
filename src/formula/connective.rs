...
impl<const ARITY: usize, OPERAND, Atom> PartialEq for DynConnective<ARITY, OPERAND, Atom>
where
    OPERAND: PartialEq,
    Atom: 'static, // because of the `UsableConnective`: `DynCompare`: `AsDynCompare`: `Any`: `'static`
{
    fn eq(&self, other: &Self) -> bool {
        (self.connective == &other.connective) && (self.operands == other.operands)
    }
}

impl<const ARITY: usize, OPERAND: PartialEq, Atom: 'static> Eq
    for DynConnective<ARITY, OPERAND, Atom>
{
}
...
mod usable {
    ...
    impl<const N: usize, Atom: 'static> PartialEq<&Self> for Box<dyn UsableConnective<N, Atom>> {
        fn eq(&self, other: &&Self) -> bool {
